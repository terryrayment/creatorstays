...
export default function HostLoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      document.cookie = `cs_session=host_${Date.now()}; path=/; max-age=86400`
      document.cookie = `cs_role=host; path=/; max-age=86400`
      document.cookie = `cs_user=${encodeURIComponent(JSON.stringify({ email: form.email, role: "host" }))}; path=/; max-age=86400`
  ...
  return (
    <div className="min-h-screen bg-black px-3 pt-16 pb-8 lg:px-4">
      <div className="mx-auto max-w-md">
        <div className="rounded-2xl border-[3px] border-black bg-white p-6 transition-transform duration-200 hover:-translate-y-1">
          <p className="text-[9px] font-black uppercase tracking-wider text-black">Host Portal</p>
          ...
          <p className="mt-2 text-[12px] font-medium text-black">
            Access your host dashboard to manage properties and creators.
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-3">
            ...
            <div className="flex items-center justify-between pt-1">
              <Link href="/login/host/forgot" className="text-[10px] font-bold text-black underline">
                Forgot password?
              </Link>
            </div>
          ...
          <div className="mt-5 border-t-[2px] border-black pt-4">
            <p className="text-[11px] font-medium text-black text-center">
              ...
              <Link href="/hosts" className="font-black underline">Sign up as Host</Link>
            </p>
          </div>
        </div>
        ...
        <p className="mt-4 text-center text-[10px] font-medium text-white/60">
          Are you a creator?{" "}
          <Link href="/login/creator" className="font-bold text-white underline">Creator login</Link>
        </p>
      </div>
    </div>
  )
